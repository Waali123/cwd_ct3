...
path = ''.join(['../data/original/shanghaitech/part_',
               dataset, '/test_data/images/'])
gt_path = ''.join(['../data/original/shanghaitech/part_',
                  dataset, '/test_data/ground-truth/'])
gt_path_csv = ''.join(['../data/original/shanghaitech/part_',
                      dataset, '/test_data/ground-truth_csv/'])
...
for i in range(1, num_images+1):
    ...
    image_info = loadmat(''.join((gt_path, 'GT_IMG_', str(i), '.mat')))[
        'image_info']
    input_img_name = ''.join((path, 'IMG_', str(i), '.jpg'))
    im = cv2.imread(input_img_name, 0)
    annPoints = image_info[0][0][0][0][0] - 1
    im_density = get_density_map_gaussian.get_density_map_gaussian(
        im, annPoints)
